...
public sealed record HotPathAllocationCase(GraphPetriNet Graph, MatrixPetriNet Matrix, Marking Marking, int ConflictPlaceId);

public static class HotPathAllocationPropertyData
{
    public static HotPathAllocationCase CreateConflictCase(int branchCount)
    {
        if (branchCount < 2)
        {
        ...
        return new HotPathAllocationCase(graph, matrix, marking, 0);
    }

    public static HotPathAllocationCase CreateFireCase(int transitionCount)
    {
